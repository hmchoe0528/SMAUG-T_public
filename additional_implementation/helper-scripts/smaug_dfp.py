...
def runSmaugError(ver, n, k, q, p, hs, hr, prob_read):
    ...
    """
    data_name = ver
    data_name = str(ver) + "_n" + str(n) + "_k" + str(k) +"_q" + str(q) + "_p" + str(p) + "_hs" + str(hs) + "_hr" + str(hr)
    ...
    # Make e2
    e2 = distList(e2_list, int(q/2/p-1))
    
    # sum_dist = <e,r> + <e1, s> + e2
    sum_dist = er + es
