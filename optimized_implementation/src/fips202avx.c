...
/*************************************************
 * Name:        keccak_init_avx
 *
 * Description: Initializes the Keccak state.
 *
 * Arguments:   - uint64_t *s: pointer to Keccak state
 **************************************************/
static void keccak_init_avx(uint64_t *s) {
    KeccakP1600_Initialize_avx2(s);
    s[25] = 0;
}

/*************************************************
 * Name:        keccak_absorb_avx
 *
 ...
 **************************************************/
static void keccak_absorb_avx(uint64_t *s, uint32_t r, const uint8_t *m,
                              size_t mlen) {
    uint64_t c;
 ...
 **************************************************/
static void keccak_finalize_avx(uint64_t *s, uint32_t r, uint8_t p) {
    /* After keccak_absorb_avx, we are guaranteed that s[25] < r,
 ...
 *              - uint32_t r: rate in bytes (e.g., 168 for SHAKE128)
 **************************************************/
static void keccak_squeeze_avx(uint8_t *h, size_t outlen, uint64_t *s,
                               uint32_t r) {
    while (outlen > s[25]) {
...
/* shake128 */
void shake128_init_avx(keccak_state_avx *state) { keccak_init_avx(state->s); }

void shake128_absorb_avx(keccak_state_avx *state, const uint8_t *input,
                         size_t inlen) {
    keccak_absorb_avx(state->s, SHAKE128_RATE, input, inlen);
}

void shake128_finalize_avx(keccak_state_avx *state) {
    keccak_finalize_avx(state->s, SHAKE128_RATE, 0x1F);
}

void shake128_squeeze_avx(uint8_t *output, size_t outlen,
                          keccak_state_avx *state) {
    keccak_squeeze_avx(output, outlen, state->s, SHAKE128_RATE);
}

void shake128_avx(uint8_t *output, size_t outlen, const uint8_t *input,
                  size_t inlen) {
    keccak_state_avx state;
    shake128_init_avx(&state);
    shake128_absorb_avx(&state, input, inlen);
    shake128_finalize_avx(&state);
    shake128_squeeze_avx(output, outlen, &state);
}

/* shake256 */
void shake256_init_avx(keccak_state_avx *state) { keccak_init_avx(state->s); }

void shake256_absorb_avx(keccak_state_avx *state, const uint8_t *input,
                         size_t inlen) {
    keccak_absorb_avx(state->s, SHAKE256_RATE, input, inlen);
}

void shake256_finalize_avx(keccak_state_avx *state) {
    keccak_finalize_avx(state->s, SHAKE256_RATE, 0x1F);
}

void shake256_squeeze_avx(uint8_t *output, size_t outlen,
                          keccak_state_avx *state) {
    keccak_squeeze_avx(output, outlen, state->s, SHAKE256_RATE);
}
...
void shake256_avx(uint8_t *output, size_t outlen, const uint8_t *input,
    ...
    shake256_absorb_avx(&state, input, inlen);
    shake256_finalize_avx(&state);
    shake256_squeeze_avx(output, outlen, &state);
}

/* sha3-256 */
void sha3_256_init_avx(keccak_state_avx *state) { keccak_init_avx(state->s); }

void sha3_256_absorb_avx(keccak_state_avx *state, const uint8_t *input,
                         size_t inlen) {
    keccak_absorb_avx(state->s, SHA3_256_RATE, input, inlen);
}

void sha3_256_finalize_avx(uint8_t *output, keccak_state_avx *state) {
    keccak_finalize_avx(state->s, SHA3_256_RATE, 0x06);
    keccak_squeeze_avx(output, 32, state->s, SHA3_256_RATE);
}

void sha3_256_avx(uint8_t *output, const uint8_t *input, size_t inlen) {
    keccak_state_avx state;
    sha3_256_init_avx(&state);
    sha3_256_absorb_avx(&state, input, inlen);
    sha3_256_finalize_avx(output, &state);
}